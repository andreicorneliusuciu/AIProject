...
//				}
				...
				if (isCabin(i - 1, j - 1, x, y) && isCabin(i - 1, j, x, y) && isCabin(i - 1, j + 1, x, y)
						&& isCabin(i + 1, j - 1, x, y) && isCabin(i + 1, j, x, y) && isCabin(i + 1, j + 1, x, y)
						&& isCabin(i, j + 1, x, y) && isCabin(i, j - 1, x, y)) {
					if (!wallSpace[i - 1][j - 1] && !wallSpace[i - 1][j] && wallSpace[i - 1][j + 1]
							&& !wallSpace[i][j - 1] && wallSpace[i][j + 1] && !wallSpace[i + 1][j - 1]
							&& !wallSpace[i + 1][j] && wallSpace[i + 1][j + 1]) {
				...
				if (isCabin(i - 1, j - 1, x, y) && isCabin(i - 1, j, x, y) && isCabin(i - 1, j + 1, x, y)
						&& isCabin(i + 1, j - 1, x, y) && isCabin(i + 1, j, x, y) && isCabin(i + 1, j + 1, x, y)
						&& isCabin(i, j + 1, x, y) && isCabin(i, j - 1, x, y)) {
					if (wallSpace[i - 1][j - 1] && wallSpace[i - 1][j] && wallSpace[i - 1][j + 1] && !wallSpace[i][j - 1]
							&& !wallSpace[i][j + 1] && !wallSpace[i + 1][j - 1] && !wallSpace[i + 1][j]
							&& !wallSpace[i + 1][j + 1]) {
						theList.add(new Position(i, j));
				...
				if (isCabin(i - 1, j - 1, x, y) && isCabin(i - 1, j, x, y) && isCabin(i - 1, j + 1, x, y)
					...
					if (!wallSpace[i - 1][j - 1] && !wallSpace[i - 1][j] && !wallSpace[i - 1][j + 1]
							&& wallSpace[i][j - 1] && wallSpace[i][j + 1] 
							&& wallSpace[i + 1][j]) {
						theList.add(new Position(i, j));
				...
				if (isCabin(i - 1, j - 1, x, y) && isCabin(i - 1, j, x, y) && isCabin(i - 1, j + 1, x, y)
						&& isCabin(i + 1, j - 1, x, y) && isCabin(i + 1, j, x, y) && isCabin(i + 1, j + 1, x, y)
						&& isCabin(i, j + 1, x, y) && isCabin(i, j - 1, x, y)) {
					if ( wallSpace[i - 1][j] && !wallSpace[i - 1][j + 1]
							&& wallSpace[i][j - 1] && !wallSpace[i][j + 1] 
							&& wallSpace[i + 1][j] && !wallSpace[i + 1][j + 1]) {
						theList.add(new Position(i, j));
				...
				if (isCabin(i - 1, j - 1, x, y) && isCabin(i - 1, j, x, y) && isCabin(i - 1, j + 1, x, y)
						&& isCabin(i + 1, j - 1, x, y) && isCabin(i + 1, j, x, y) && isCabin(i + 1, j + 1, x, y)
						&& isCabin(i, j + 1, x, y) && isCabin(i, j - 1, x, y)) {
					if (wallSpace[i - 1][j] && wallSpace[i][j - 1]
							&& wallSpace[i][j + 1] && !wallSpace[i + 1][j - 1] && !wallSpace[i + 1][j]
							&& !wallSpace[i + 1][j + 1]) {
						theList.add(new Position(i, j));
				...
				if (isCabin(i - 1, j - 1, x, y) && isCabin(i - 1, j, x, y) && isCabin(i - 1, j + 1, x, y)
						&& isCabin(i + 1, j - 1, x, y) && isCabin(i + 1, j, x, y) && isCabin(i + 1, j + 1, x, y)
						&& isCabin(i, j + 1, x, y) && isCabin(i, j - 1, x, y)) {
					if (!wallSpace[i - 1][j - 1] && wallSpace[i - 1][j] 
							&& !wallSpace[i][j - 1] && wallSpace[i][j + 1] && !wallSpace[i + 1][j - 1]
							&& wallSpace[i + 1][j] ) {
						theList.add(new Position(i, j));
	...
	public int h(Node n) {
		...
		while (it1.hasNext() && it2.hasNext()) {
			...
			Box b = it2.next();
			Goal g = it1.next();
			result += DistancesComputer.getDistanceBetween2Positions(b.position, g.position);// here???
			// System.err.println("Dist between " + b.position + " and " +
			// g.position + " is " + result);
			// System.err.println("HEURISTIC} goals2 has size = " +
			// n.goals2.size());
			// System.err.println("HEURISTIC} boxesOrderedAlphabetically has
			// size = " + boxesOrderedAlphabetically.size());
			// result += DistancesComputer.
			// getDistanceBetween2Positions(b.position, new Position(n.agentRow,
			// n.agentCol));
		}
		...
		for (int i = 0; i < DistancesComputer.levelRowSize - 2; i++) {
			for (int j = 0; j < DistancesComputer.levelColSize - 2; j++) {
				...
				if ('A' <= n.boxes[i][j] && n.boxes[i][j] <= 'Z'
						&& n.myBoxesFinal.contains(new Box(n.boxes[i][j], n.theAgentColor, new Position(i, j)))) {
